...
test.describe('New Quote Evaluation Flow', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.getByTestId('new-quote-header')).toBeVisible({
      timeout: 10000,
    });
  ...
  test('should pre-populate client fields using active user session data', async ({
    page,
  }) => {
  ...
  test('should dynamically update live estimates sidebar when form fields become valid', async ({
    page,
  }) => {
    ...
    await page
      .getByTestId('new-quote-input-address')
      .fill('Müllerstraße 42, 13353 Berlin');
    await page
      .getByTestId('new-quote-input-monthly-consumption-kwh')
      .fill('450');
  ...
  test('should handle validation error rules on form submission failure', async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('text=String must contain at least')
    ).toBeVisible();
    await expect(page.locator('text=Invalid email')).toBeVisible();
  });

  test('should successfully submit form data and route directly to the generated quote view', async ({
    page,
  }) => {
    ...
    await expect(page.getByTestId('new-quote-input-submit')).toHaveText(
      'Running Underwriting Engine...'
    );
  ...
  test('should display server error alert banners when API responds with an explicit error', async ({
    page,
  }) => {
