...
export async function POST(req: Request) {
  ...
  if (!session?.user?.id) {
    logger.warn(
      'Unauthorized resource request blocked: No active session context parsed.'
    );
  ...
  try {
    ...
    if (!validationResult.success) {
      ...
      return NextResponse.json(
        {
          error: 'Invalid form input data parameters structural layout',
    ...
    return NextResponse.json(
      {
        error:
          'Internal processing anomaly tripped within underlying database service engine.',
      },
      { status: 500 }
    );
  }
