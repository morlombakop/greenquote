...
export const generateQuotePdf = (quote: PdfQuoteData, parsedOffers: InstallmentOffer[]) => {
  const doc = new jsPDF({
  ...
  autoTable(doc, {
    ...
    body: [
      ["Customer Name:", quote.fullName, "System Dimension:", `${quote.systemSizeKw.toFixed(2)} kWp`],
      ["Email Address:", quote.email, "Monthly Usage:", `${quote.monthlyConsumptionKwh} kWh`],
      ["Installation Address:", quote.address, "Gross System Price:", `€${quote.systemPrice.toLocaleString("de-DE", { minimumFractionDigits: 2 })}`],
      ["Credit Risk Band:", `Band ${quote.riskBand}`, "Financing Principal:", `€${quote.principalAmount.toLocaleString("de-DE", { minimumFractionDigits: 2 })}`],
    ],
  });
  ...
  autoTable(doc, {
    ...
    head: [["Amortization Term", "APR %", "Principal Base", "Monthly Installment", "Aggregate Total Cost"]],
    body: tableRows,
  });
  ...
  doc.save(`GreenQuote_${quote.fullName.replace(/\s+/g, "_")}_${quote.id.slice(0, 8)}.pdf`);
};
